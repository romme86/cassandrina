...
Discovers a BTC threshold market near the user-favored edge, then builds a
minority modulation signal from probability, recent trade flow, and recent
price momentum.
"""
...
from cassandrina.decision_engine import PolymarketSignal
...
class PolymarketClient:
    ...
    def fetch_btc_probability(self, target_date: date) -> float:
        """
        Legacy probability fetch retained for compatibility and tests.
        """
        try:
    ...
    def _matches_target_date(self, market: dict, target_date: date) -> bool:
        ...
        return (
            date_str_full in question
            or (date_str_month in question and date_str_year in question)
        )
